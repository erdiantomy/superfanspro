...
function NotificationsTab({ notifs, unreadCount, markAllRead }: { notifs: any[]; unreadCount: number; markAllRead: () => void }) {
  ...
  const SortHeader = ({ col, label }: { col: string; label: string }) => (
    <th
      onClick={() => setPlayerSort(prev => ({ col, asc: prev.col === col ? !prev.asc : false }))}
      style={{ padding: "8px 6px", fontSize: 10, fontWeight: 700, color: playerSort.col === col ? C.orange : C.muted, cursor: "pointer", textTransform: "uppercase", letterSpacing: 0.5, whiteSpace: "nowrap", userSelect: "none" }}
    >
      {label} {playerSort.col === col ? (playerSort.asc ? "↑" : "↓") : ""}
    </th>
  );
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #1E2235" }}>
                ...
                <SortHeader col="slug" label="Slug" />
                <SortHeader col="games_played" label="Games" />
                <SortHeader col="win_rate" label="Win %" />
                <SortHeader col="total_raised" label="Raised" />
                <SortHeader col="supporter_count" label="Fans" />
                <th style={{ padding: "8px 6px", fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase" }}>Status</th>
                <SortHeader col="profile_created_at" label="Created" />
              </tr>
            </thead>
            <tbody>
              {sortedPlayers.map((p: any) => (
                <tr key={p.profile_id} style={{ borderBottom: "1px solid #1E2235" }}>
                  ...
                  <td style={{ padding: "10px 6px" }}>
                    <a href={`/${p.slug}`} target="_blank" rel="noopener" style={{ color: C.green, textDecoration: "none", fontWeight: 600 }}>/{p.slug}</a>
                  </td>
                  <td style={{ padding: "10px 6px", textAlign: "center" }}>{p.games_played ?? 0}</td>
                  <td style={{ padding: "10px 6px", textAlign: "center" }}>{p.win_rate ?? 0}%</td>
                  <td style={{ padding: "10px 6px", textAlign: "right", color: C.green, fontWeight: 600 }}>Rp {((p.total_raised || 0) as number).toLocaleString("id-ID")}</td>
                  ...
                  <td style={{ padding: "10px 6px", textAlign: "center" }}>
                    <Tag label={p.is_public ? "PUBLIC" : "PRIVATE"} color={p.is_public ? C.green : C.muted} />
                  </td>
                  <td style={{ padding: "10px 6px", fontSize: 10, color: C.dim }}>{p.profile_created_at ? new Date(p.profile_created_at).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "—"}</td>
                </tr>
              ))}
  ...
  // Render players tab in content area
  if (tab === "players") return playersContent;
