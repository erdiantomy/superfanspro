...
import { useMonthlyLeaderboard, useLifetimeLeaderboard, useSessions } from "@/hooks/useArena";
...
export default function Landing() {
  ...
  // Realtime — all screens auto-refresh on DB changes
  useArenaRealtime();

  const { data: monthly  = [], isLoading: mLoad } = useMonthlyLeaderboard();
  const { data: lifetime = [] }                    = useLifetimeLeaderboard();
  const { data: sessions = [] } = useSessions(["live", "active"]);
    ...
    <div className="min-h-screen bg-background text-foreground max-w-md mx-auto" style={{ height: "100dvh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, flexShrink: 0, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: 9, color: C.dim, letterSpacing: 1 }}>tomspadel.com · LIVE RANKINGS</div>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {user ? (
            <>
              <button onClick={() => navigate("/host")} style={{ background: `${C.green}18`, border: `1px solid ${C.green}40`, color: C.green, padding: "6px 12px", borderRadius: 10, fontFamily: "'Barlow Condensed'", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>+ SESSION</button>
              <button onClick={() => navigate("/admin")} style={{ background: `${C.orange}15`, border: `1px solid ${C.orange}40`, color: C.orange, padding: "6px 10px", borderRadius: 10, fontFamily: "'Barlow Condensed'", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>🛡</button>
            </>
          ) : (
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px 90px" }}>
        ...
        {/* Prize banner */}
        <div style={{ background: "linear-gradient(135deg,#0B1A0C,#0B0E16)", border: `1px solid ${C.green}25`, borderRadius: 14, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 10, color: C.muted, letterSpacing: 1, textTransform: "uppercase" }}>March 2026 · Prize Season</div>
            <div className="font-display" style={{ fontSize: 24, fontWeight: 900, color: C.green }}>Rp 2.000.000</div>
            <div style={{ fontSize: 10, color: C.muted }}>🥇 1jt · 🥈 600k · 🥉 400k</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 10, color: C.muted }}>ends in</div>
            <div className="font-display" style={{ fontSize: 28, color: C.red, lineHeight: 1 }}>9D</div>
          </div>
        </div>
        ...
        {live.length > 0 && (
          <>
            ...
            {live.map(s => (
              <div key={s.id} onClick={() => navigate(`/session/${s.code}`)} style={{ background: "linear-gradient(160deg,#0D1E14,#0A0C11)", border: `1px solid ${C.green}35`, borderRadius: 16, padding: 14, marginBottom: 10, cursor: "pointer" }}>
                ...
                <div style={{ fontSize: 11, color: C.green, fontWeight: 600 }}>Tap to view session →</div>
              </div>
            ))}
          </>
        )}
        ...
        {upcoming.length > 0 && (
          <>
            ...
            {upcoming.map(s => (
              <div key={s.id} onClick={() => navigate(`/session/${s.code}`)} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "12px 14px", marginBottom: 8, cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          {([{ v: "monthly", l: "🏅 Monthly Pts" }, { v: "lifetime", l: "⚡ Lifetime XP" }] as const).map(t => (
            <button key={t.v} onClick={() => setRankTab(t.v)} style={{ flex: 1, padding: "8px 0", borderRadius: 20, background: rankTab === t.v ? `${C.green}15` : C.card, border: `1px solid ${rankTab === t.v ? C.green + "40" : C.border}`, color: rankTab === t.v ? C.green : C.muted, fontFamily: "'Barlow Condensed'", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{t.l}</button>
          ))}
        </div>
          ...
          list.map((p, i) => {
            ...
            return (
              <motion.div key={p.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 14, marginBottom: 6, background: C.card, border: `1px solid ${C.border}` }}>
                ...
                <div style={{ textAlign: "right" }}>
                  <div className="font-display" style={{ fontSize: 17, fontWeight: 900, color: i === 0 ? C.green : C.muted }}>{val.toLocaleString()}</div>
